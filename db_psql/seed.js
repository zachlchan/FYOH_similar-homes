...
const similar_homes_path = path.join(__dirname, '../data_generator/csv/psql_similar_home_data.csv');
...
db.client.query(
  `COPY trelia.similar_homes(listing_id, similar_id, similarity_weight) FROM '${similar_homes_path}' DELIMITER ',' CSV HEADER`,
  (err, res) => {
    if (err) {
      console.log('error seeding psql', err);
    } else {
